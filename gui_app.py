...
class TranscriptionProcessorApp(DownloadMixin, ProcessingMixin, MonitorMixin, SearchMixin, ConfigMixin, DashboardMixin):
    ...
    def _finish_init(self):
        ...
        if KnowledgeIntegrator:
            try:
                # Intentar inicializar sin configuración de IA primero
                temp_integrator = KnowledgeIntegrator(self.blacklist, db_manager=self.knowledge_db)

                # Asignar configuración si existe
                if hasattr(self, 'ai_provider'):
                    temp_integrator.ai_provider = getattr(self, 'ai_provider', 'none')
                if hasattr(self, 'api_key'):
                    temp_integrator.api_key = getattr(self, 'api_key', None)
                if hasattr(self, 'ai_system_prompt'):
                    temp_integrator.system_prompt = getattr(self, 'ai_system_prompt', '')

                # Forzar modo local (sin IA) por seguridad - evitar errores de imagen
                temp_integrator.ai_provider = "none"

                self.integrator = temp_integrator
                
                # Inicialización del Distribuidor Elite (Fase 1)
                if KBDistributor:
                ...
                self.analytics = AnalyticsEngine(self.db_manager) if AnalyticsEngine else None
            except Exception as e:
                # Si falla la inicialización, continuar sin integrador
                print(f"[INFO] KnowledgeIntegrator no disponible: {e}")
        ...
        if self._theme_manager is None:
            try:
                ...
                from app.core.ui_framework import (
                    ...
                    ResponsiveManager, BindingManager, ui_context
                ) # type: ignore
                from app.core.plugin_manager import PluginManager
    ...
    def show_about(self):
        ...
        tab_main = ttk.Frame(nb, padx=15, pady=15)
        ...
        stats_frame = ttk.LabelFrame(tab_main, text=" Impacto del Sistema ", padx=10, pady=10)
        ...
        tab_tech = ttk.Frame(nb, padx=15, pady=15)
        ...
        tab_ai = ttk.Frame(nb, padx=15, pady=15)
        ...
        footer = ttk.Frame(about_win, padx=10, pady=10)
        footer.pack(fill=tk.X)
        ttk.Label(footer, text="© 2026 KDP Master Solutions - 100% Elite Standards", font=("Segoe UI", 8)).pack(side=tk.LEFT)
        ttk.Button(footer, text="Cerrar", command=about_win.destroy).pack(side=tk.RIGHT)
    ...
    def _show_manual_not_found_dialog(self, searched_paths):
        ...
        ttk.Button(btn_frame, text="🌐 Descargar Online", command=download_online).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="📝 Crear Básico", command=create_basic).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="🔍 Ver Rutas", command=show_searched_paths).pack(side=tk.LEFT, padx=5)
...
"""
    ...
    def create_ui(self):
        ...
        self.health_btn = ttk.Button(header_frame, text="🔄 VERIFICAR SISTEMA", width=18, command=self.run_health_check)
        ...
        self.tab_channel_monitor = ttk.Frame(self.notebook, padx=10, pady=10)
        ...
        self.tab_dashboard = ttk.Frame(self.notebook, padx=10, pady=10)
        self.notebook.add(self.tab_dashboard, text=" 📊 Dashboard ")
        self.setup_dashboard_tab()
        
        # Pestaña 11: Videos Pendientes (Explorador Masivo - Pilar 1)
        self.tab_pending_mass = ttk.Frame(self.notebook, padx=10, pady=10)
        self.notebook.add(self.tab_pending_mass, text=" 🎥 Videos Pendientes ")
        self.setup_pending_videos_tab()
        
        # Pestaña 9: Cola de Revisión Humana (Fase 2 Elite)
        self.tab_review = ttk.Frame(self.notebook, padx=10, pady=10)
        ...
        self.tab_schedule = ttk.Frame(self.notebook, padx=10, pady=10)
        self.notebook.add(self.tab_schedule, text=" 📅 Programación ")
        self.setup_schedule_tab()
        
        # Pestaña 8: Configuración
        try:
            self.tab_settings = ttk.Frame(self.notebook, padx=10, pady=10)
        ...
        log_frame = ttk.LabelFrame(main_frame, text=" Terminal de Operaciones en Tiempo Real ", padx=5, pady=5)
    ...
    def _animate_tab_transition(self, event):
        ...
        def fade(alpha):
            if alpha <= 1.0:
                # Simulación de fade mediante ajuste de color de fondo si el widget lo soporta
                # o mediante el AnimationEngine si está disponible.
                if self.theme_manager:
                    self.theme_manager.apply_alpha_to_widget(selected_tab, alpha)
    ...
    def setup_download_tab(self):
        ...
        self.new_videos_frame = ttk.LabelFrame(self.tab_download, text=" 🔔 Videos Nuevos Detectados ", padx=10, pady=10)
    ...
    def setup_analyze_tab(self):
        ...
        report_frame = ttk.LabelFrame(self.tab_analyze, text=" 📊 Reporte de Integración Atmosférica ", padx=10, pady=10)
    ...
    def setup_search_tab(self):
        ...
        search_main = ttk.Frame(self.tab_search, padx=15, pady=15)
        ...
        results_frame = ttk.LabelFrame(search_main, text=" 📄 Fragmentos de Conocimiento Encontrados ", padx=10, pady=10)
    ...
    def setup_pending_videos_tab(self):
        ...
        main = ttk.Frame(self.tab_pending_mass, padx=15, pady=15)
        ...
        search_grp = ttk.LabelFrame(header, text=" 🔍 Búsqueda en Lista Local ", padding=5)
        search_grp.pack(side=tk.LEFT, padx=(0, 15))
        ttk.Entry(search_grp, textvariable=self.pending_search_var, width=35).pack(padx=5, pady=5)
        self.pending_search_var.trace("w", lambda *a: self.filter_pending_videos())

        # Filtro de Fecha (Módulo 7)
        date_grp = ttk.LabelFrame(header, text=" 📅 Filtro Temporal ", padding=5)
        date_grp.pack(side=tk.LEFT, padx=(0, 15))
        self.pending_year_var = tk.StringVar(value="Todos")
        years = ["Todos", "2024", "2025", "2026"]
        ttk.Combobox(date_grp, textvariable=self.pending_year_var, values=years, state="readonly", width=10).pack(padx=5, pady=5)
        ...
        ttk.Combobox(date_grp, textvariable=self.pending_month_var, values=months, state="readonly", width=10).pack(padx=5, pady=5)
        self.pending_month_var.trace("w", lambda *a: self.filter_pending_videos())

        # Pilar 6: Filtros de Integración KDP (Módulos 51, 53, 54, 59)
        kdp_filters_grp = ttk.LabelFrame(header, text=" 🚀 Filtros KDP ", padding=5)
        ...
        validation_grp = ttk.LabelFrame(header, text=" 🛡️ Filtros de Validación ", padding=5)
        ...
        selection_grp = ttk.LabelFrame(header, text=" 🧠 Selección Inteligente ", padding=5)
        ...
        throttle_grp = ttk.LabelFrame(header, text=" 🛡️ Throttling (Pilar 3) ", padding=5)
        ...
        # Botón Validación Online (Módulos 16-20)
        ttk.Button(header, text="🌐 Validar Disponibilidad", command=self.validate_online_selected).pack(side=tk.RIGHT, padx=5, pady=5)

        # Exportación (Módulo 10)
        ttk.Button(header, text="📤 Exportar Metadata (CSV)", command=self.export_pending_to_csv).pack(side=tk.RIGHT, pady=5)
        ttk.Button(header, text="🔄 Sincronizar DB", command=self.load_pending_videos).pack(side=tk.RIGHT, padx=10, pady=5)
        ...
        ttk.Button(list_container, text="🔄 Selección Inversa", command=self.invert_pending_selection).pack(side=tk.BOTTOM, pady=5)

        # Módulo 37: Guardar Selección como "Grupo"
        ttk.Button(list_container, text="💾 Guardar Selección (Grupo)", command=self.save_pending_selection_group).pack(side=tk.BOTTOM, pady=5)
        ...
        self.pending_preview_frame = ttk.LabelFrame(body_paned, text=" 👁️ Vista Previa (Módulo 4) ", padx=10, pady=10)
        ...
        ai_frame = ttk.LabelFrame(self.pending_preview_frame, text=" 🤖 Inteligencia Artificial (Ollama) ", padx=10, pady=10)
    ...
    def load_pending_thumbnail_async(self, video_id, url):
        ...
        ttk.Button(pag_frame, text="◀", width=4, command=lambda: self.change_pending_page(-1)).pack(side=tk.LEFT)
        self.pending_page_lbl = ttk.Label(pag_frame, text="Página 1", width=12, anchor="center", font=("Segoe UI", 9, "bold"))
        self.pending_page_lbl.pack(side=tk.LEFT, padx=10)
        ttk.Button(pag_frame, text="▶", width=4, command=lambda: self.change_pending_page(1)).pack(side=tk.LEFT)

        # Cargar Más (Módulo 6)
        ttk.Button(footer, text="➕ Aumentar Buffer de Carga", command=self.load_more_pending_size).pack(side=tk.LEFT, padx=30)
...
"""
    ...
    def validate_online_selected(self):
        ...
        def setup_review_tab(self):
                """Configura la UI para Human-in-the-Loop (Fase 2)."""
        main = ttk.Frame(self.tab_review, padx=15, pady=15)
        main.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(main, text="👁️ Entradas Pendientes de Validación IA", style="Header.TLabel").pack(anchor=tk.W)
        ttk.Label(main, text="Confianza < 0.75 | Requiere aprobación humana antes de distribución especializada.", 
                 font=("Segoe UI", 9), foreground="#94a3b8").pack(anchor=tk.W, pady=(0, 15))

        # Lista de revisión
        cols = ("id", "source", "category", "confidence")
        self.review_tree = ttk.Treeview(main, columns=cols, show="headings", height=8)
        self.review_tree.heading("id", text="ID")
        self.review_tree.heading("source", text="Fuente")
        self.review_tree.heading("category", text="Categoría Sugerida")
        self.review_tree.heading("confidence", text="Confianza IA")
        self.review_tree.pack(fill=tk.X, pady=5)

        # Área de edición
        edit_frame = ttk.LabelFrame(main, text=" Contenido del Snippet ", padx=10, pady=10)
        edit_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        self.review_text = scrolledtext.ScrolledText(edit_frame, height=10, font=("Consolas", 10))
        ...
        ttk.Button(btn_row, text="✅ Aprobar y Distribuir", style="Success.TButton", 
                  command=lambda: self.log("Aprobado")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_row, text="✏️ Corregir Categoría", 
                  command=lambda: self.log("Editado")).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_row, text="🗑️ Descartar", style="Danger.TButton",
                  command=lambda: self.log("Descartado")).pack(side=tk.RIGHT, padx=5)

        self.log("✅ Pestaña de Revisión Humana activada.")
    ...
    def run_first_time_wizard(self):
        ...
        frame = ttk.Frame(win, padx=20, pady=20)
    ...
    def _show_doc_updater_dialog(self):
        ...
        form_frame = ttk.Frame(dialog, padx=15, pady=15)
        ...
        doc_entry.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
        ...
        feature_id_entry.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Button(dialog, text="Actualizar Documento", command=perform_update, style="Primary.TButton").pack(pady=20)
    ...
    def _show_export_settings(self):
        ...
        settings_frame = ttk.LabelFrame(win, text="Opciones", padx=15, pady=15)
    ...
    def setup_channel_monitor_tab(self):
        ...
        drop_zone_frame = ttk.LabelFrame(main_container, text=" ☁️ Añadir Canal Rápido ", padx=15, pady=15)
        ...
        list_frame = ttk.LabelFrame(left_panel, text=" 📋 Canales Monitoreados ", padx=10, pady=10)
        ...
        control_frame = ttk.LabelFrame(right_panel, text=" ⚙️ Control de Monitoreo ", padx=15, pady=15)
        control_frame.pack(fill=tk.X, pady=(0, 15))
        ...
        # Log de actividad
        log_frame = ttk.LabelFrame(right_panel, text=" 📜 Actividad del Monitor ", padx=10, pady=10)
        ...
        filter_panel = ttk.LabelFrame(right_panel, text=" 🔍 Filtros por Palabras Clave ", padx=10, pady=10)
    ...
    def show_monitor_stats(self):
        ...
        main = ttk.Frame(win, padx=20, pady=20)
        main.pack(fill=tk.BOTH, expand=True)
    ...
    def setup_settings_tab(self):
        """
        MÓDULO 1: Configuración Robusta con ttk + sv-ttk.
        Elimina dependencia de customtkinter para evitar conflictos de renderizado.
        Prioriza: Directorios, Tema, IA y Filtros.
        """
        # === MÓDULO 1A: Diagnóstico con try-except robusto ===
        try:
            import sv_ttk
        except ImportError:
            sv_ttk = None
            self.log("⚠️ sv-ttk no disponible, usando estilos estándar")
        
        try:
            # === MÓDULO 1B: Contenedor principal con scroll ===
            main_container = ttk.Frame(self.tab_settings, padx=20, pady=20)
            main_container.pack(fill=tk.BOTH, expand=True)
            
            # Canvas para scroll vertical
            canvas = tk.Canvas(main_container, bg="#1e293b" if self.current_theme == "dark" else "#f8fafc", 
            ...
            # === MÓDULO 2: SECCIÓN 1 - DIRECTORIOS DE TRABAJO (CRÍTICO) ===
            dir_frame = ttk.LabelFrame(scrollable_frame, text=" 📁 Directorios de Trabajo ", padx=15, pady=15)
            dir_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            # Input Dir
            ttk.Label(dir_frame, text="Carpeta de Entrada (Transcripciones):", font=('Segoe UI', 9, 'bold')).grid(row=0, column=0, sticky=tk.W, pady=5)
            current_input = self.input_dir.get() if hasattr(self, 'input_dir') else "data/transcriptions"
            self.settings_input_dir_var = tk.StringVar(value=current_input)
            entry_in = ttk.Entry(dir_frame, textvariable=self.settings_input_dir_var, width=50)
            entry_in.grid(row=1, column=0, sticky=tk.EW, padx=(0, 10), pady=5)
            btn_in = ttk.Button(dir_frame, text="🔍", width=8, command=lambda: self._browse_dir_settings(self.settings_input_dir_var))
            btn_in.grid(row=1, column=1, pady=5)

            # Output Dir
            ttk.Label(dir_frame, text="Carpeta de Salida (Procesados):", font=('Segoe UI', 9, 'bold')).grid(row=2, column=0, sticky=tk.W, pady=(10, 5))
            current_output = self.output_dir.get() if hasattr(self, 'output_dir') else "outputs/processed"
            self.settings_output_dir_var = tk.StringVar(value=current_output)
            entry_out = ttk.Entry(dir_frame, textvariable=self.settings_output_dir_var, width=50)
            entry_out.grid(row=3, column=0, sticky=tk.EW, padx=(0, 10), pady=5)
            btn_out = ttk.Button(dir_frame, text="🔍", width=8, command=lambda: self._browse_dir_settings(self.settings_output_dir_var))
            btn_out.grid(row=3, column=1, pady=5)
            
            dir_frame.columnconfigure(0, weight=1)

            # === MÓDULO 3: SECCIÓN 2 - APARIENCIA Y TEMA ===
            theme_frame = ttk.LabelFrame(scrollable_frame, text=" 🎨 Apariencia ", padx=15, pady=15)
            theme_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Label(theme_frame, text="Tema de la Interfaz:").grid(row=0, column=0, sticky=tk.W, pady=5)
            self.theme_choice_var = tk.StringVar(value=self.current_theme)
            theme_combo = ttk.Combobox(theme_frame, textvariable=self.theme_choice_var, 
                                      values=["dark", "light"], state="readonly", width=20)
            theme_combo.grid(row=0, column=1, sticky=tk.W, padx=10)
            theme_combo.bind("<<ComboboxSelected>>", lambda e: self.apply_specific_theme(self.theme_choice_var.get()))
            
            ttk.Label(theme_frame, text="(Cambio inmediato)", font=('Segoe UI', 8), foreground="#94a3b8").grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=(5, 0))

            # === MÓDULO 4: SECCIÓN 3 - INTELIGENCIA ARTIFICIAL (KDP CONTEXT) ===
            ai_frame = ttk.LabelFrame(scrollable_frame, text=" 🤖 Configuración de IA (Análisis KDP) ", padx=15, pady=15)
            ai_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            # Proveedor
            ttk.Label(ai_frame, text="Proveedor de IA:").grid(row=0, column=0, sticky=tk.W, pady=5)
            current_provider = getattr(self, 'ai_provider', 'ollama')
            self.ai_provider_var = tk.StringVar(value=current_provider)
            provider_combo = ttk.Combobox(ai_frame, textvariable=self.ai_provider_var, 
                                        values=["ollama", "openai", "gemini", "none"], state="readonly", width=20)
            provider_combo.grid(row=0, column=1, sticky=tk.W, padx=10)
            
            # API Key
            ttk.Label(ai_frame, text="API Key (OpenAI/Gemini):").grid(row=1, column=0, sticky=tk.W, pady=(10, 5))
            current_key = getattr(self, 'api_key', '')
            self.ai_api_key_var = tk.StringVar(value=current_key)
            entry_key = ttk.Entry(ai_frame, textvariable=self.ai_api_key_var, width=40, show="•")
            entry_key.grid(row=1, column=1, sticky=tk.W, padx=10, pady=5)
            
            # Modelo Ollama
            ttk.Label(ai_frame, text="Modelo Local (Ollama):").grid(row=2, column=0, sticky=tk.W, pady=(10, 5))
            current_model = getattr(self, 'ollama_model', 'qwen2.5:7b')
            self.ollama_model_var = tk.StringVar(value=current_model)
            entry_model = ttk.Entry(ai_frame, textvariable=self.ollama_model_var, width=30)
            entry_model.grid(row=2, column=1, sticky=tk.W, padx=10, pady=5)
            ttk.Label(ai_frame, text="Ej: qwen2.5:7b, llama3.1, mistral", font=('Segoe UI', 8), foreground="#94a3b8").grid(row=3, column=1, sticky=tk.W)

            # === MÓDULO 5: SECCIÓN 4 - FILTROS DE PALABRAS CLAVE (KDP SPECIFIC) ===
            filter_frame = ttk.LabelFrame(scrollable_frame, text=" 🔍 Filtros de Contenido (Spam/Ruido) ", padx=15, pady=15)
            filter_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Label(filter_frame, text="Palabras a Excluir (separadas por coma):", font=('Segoe UI', 9, 'bold')).pack(anchor=tk.W)
            ttk.Label(filter_frame, text="Ej: suscríbete, like, campanita, patrocinado", font=('Segoe UI', 8), foreground="#94a3b8").pack(anchor=tk.W, pady=(0, 5))
            
            current_blacklist = ", ".join(getattr(self, 'blacklist', []))
            self.filter_blacklist_var = tk.StringVar(value=current_blacklist)
            filter_text = tk.Text(filter_frame, height=4, width=60, font=('Consolas', 9))
            filter_text.insert(tk.END, current_blacklist)
            filter_text.pack(fill=tk.X, pady=5)
            
            # === [MODULE: MONITOR_CHANNELS] START ===
            monitor_frame = ttk.LabelFrame(scrollable_frame, text=" 📺 Monitor de Canales ", padx=15, pady=15)
            monitor_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Label(monitor_frame, text="Frecuencia de Escaneo:").grid(row=0, column=0, sticky=tk.W, pady=5)
            self.scan_frequency_var = tk.StringVar(value=str(getattr(self, 'scan_frequency', 30)))
            freq_combo = ttk.Combobox(monitor_frame, textvariable=self.scan_frequency_var, 
                                         values=["15", "30", "60", "120", "240"], state="readonly", width=15)
            freq_combo.grid(row=0, column=1, sticky=tk.W, padx=10)
            ttk.Label(monitor_frame, text="minutos", font=('Segoe UI', 8), foreground="#94a3b8").grid(row=0, column=2, sticky=tk.W, padx=5)
            
            ttk.Label(monitor_frame, text="Notificaciones:", font=('Segoe UI', 9, 'bold')).grid(row=1, column=0, sticky=tk.W, pady=(15, 5))
            self.notif_enabled_var = tk.BooleanVar(value=getattr(self, 'notifications_enabled', True))
            notif_check = ttk.Checkbutton(monitor_frame, text="Habilitar notificaciones", variable=self.notif_enabled_var)
            notif_check.grid(row=1, column=1, sticky=tk.W, padx=10)
            
            channel_count = len(getattr(self, 'channels', []))
            ttk.Label(monitor_frame, text="Canales Vigilados:", font=('Segoe UI', 9, 'bold')).grid(row=2, column=0, sticky=tk.W, pady=(15, 5))
            ttk.Label(monitor_frame, text=f"{channel_count} canales", font=('Segoe UI', 9), foreground="#10b981").grid(row=2, column=1, sticky=tk.W, padx=10)
            ttk.Button(monitor_frame, text="Gestionar", command=self.manage_channels).grid(row=2, column=2, padx=5)

            # === [INICIO FUNCIONALIDAD US-001: CONTROLES DE ESCANEO] ===
            ttk.Label(monitor_frame, text="Profundidad de Escaneo:").grid(row=3, column=0, sticky=tk.W, pady=5)
            self.max_results_var = tk.StringVar(value=str(getattr(self, 'max_results_per_check', 50)))
            depth_combo = ttk.Combobox(monitor_frame, textvariable=self.max_results_var, 
                                         values=["20", "50", "100"], state="readonly", width=15)
            depth_combo.grid(row=3, column=1, sticky=tk.W, padx=10)
            ttk.Label(monitor_frame, text="videos/canal", font=('Segoe UI', 8), foreground="#94a3b8").grid(row=3, column=2, sticky=tk.W, padx=5)

            # --- INICIO FUNCIONALIDAD US-038-UI: CONTROLES ADICIONALES DE MONITOR ---
            ttk.Checkbutton(monitor_frame, text="Excluir Shorts (< 60s)", 
                           variable=self.exclude_shorts_var).grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=5)
            
            ttk.Checkbutton(monitor_frame, text="Intervalo Adaptativo por Prioridad", 
                           variable=self.adaptive_interval_var).grid(row=6, column=0, columnspan=2, sticky=tk.W, pady=5)
            # --- FIN FUNCIONALIDAD ---

            ttk.Label(monitor_frame, text="Antigüedad Máxima:").grid(row=4, column=0, sticky=tk.W, pady=5)
            self.max_age_var = tk.StringVar(value=str(getattr(self, 'max_age_days', 7)))
            age_spin = ttk.Spinbox(monitor_frame, from_=1, to=365, textvariable=self.max_age_var, width=14)
            age_spin.grid(row=4, column=1, sticky=tk.W, padx=10)
            ttk.Label(monitor_frame, text="días", font=('Segoe UI', 8), foreground="#94a3b8").grid(row=4, column=2, sticky=tk.W, padx=5)
            # === [FIN FUNCIONALIDAD US-001: CONTROLES DE ESCANEO] ===
            # === [MODULE: MONITOR_CHANNELS] END ===
            
            # === [MODULE: EXPORT_SETTINGS] START ===
            export_frame = ttk.LabelFrame(scrollable_frame, text=" 📤 Exportación y Reportes ", padx=15, pady=15)
            export_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Label(export_frame, text="Formato Predeterminado:").grid(row=0, column=0, sticky=tk.W, pady=5)
            ...
            format_combo = ttk.Combobox(export_frame, textvariable=self.export_format_var, 
                                         values=["html", "pdf", "markdown"], state="readonly", width=15)
            format_combo.grid(row=0, column=1, sticky=tk.W, padx=10)
            
            ttk.Label(export_frame, text="WeasyPrint:").grid(row=1, column=0, sticky=tk.W, pady=(15, 5))
            weasy_installed = self._check_weasyprint_installed()
            weasy_status = "✅ Instalado" if weasy_installed else "❌ No instalado"
            ttk.Label(export_frame, text=weasy_status, foreground="#10b981" if weasy_installed else "#ef4444").grid(row=1, column=1, sticky=tk.W, padx=10)
            if not weasy_installed:
                ttk.Button(export_frame, text="Instalar", command=self._install_weasyprint).grid(row=1, column=2, padx=5)
            
            self.auto_export_var = tk.BooleanVar(value=getattr(self, 'auto_export_on_exit', False))
            auto_export_check = ttk.Checkbutton(export_frame, text="Auto-exportar al cerrar", variable=self.auto_export_var)
            auto_export_check.grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=(15, 5), padx=10)
            # === [MODULE: EXPORT_SETTINGS] END ===
            
            # --- INICIO FUNCIONALIDAD US-E-UI: PANEL AVANZADO DE EXPORTACIÓN v3.4.6 ---
            adv_export_frame = ttk.LabelFrame(scrollable_frame, text=" 🚀 Exportación Advanced Elite ", padx=15, pady=15)
            adv_export_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Checkbutton(adv_export_frame, text="Compresión ZIP Automática", 
                           variable=self.export_zip_var).grid(row=0, column=0, sticky=tk.W, pady=5)
            
            ttk.Checkbutton(adv_export_frame, text="Modo Incremental (Solo nuevos)", 
                           variable=self.export_incremental_var).grid(row=1, column=0, sticky=tk.W, pady=5)
            
            ttk.Label(adv_export_frame, text="Programación Automática:").grid(row=2, column=0, sticky=tk.W, pady=5)
            sched_combo = ttk.Combobox(adv_export_frame, textvariable=self.export_auto_sched_var,
                                      values=["none", "daily", "weekly"], state="readonly", width=15)
            sched_combo.grid(row=2, column=1, sticky=tk.W, padx=10)
            
            ttk.Button(adv_export_frame, text="📜 Ver Historial DB", 
                      command=self._show_export_history).grid(row=3, column=0, pady=10, sticky=tk.W)
            
            ttk.Button(adv_export_frame, text="👁️ Preview Contenido", 
                      command=self._preview_kb_export).grid(row=3, column=1, pady=10, sticky=tk.W)
            # --- FIN FUNCIONALIDAD ---
            
            # === [MODULE: SECURITY_SETTINGS] START ===
            security_frame = ttk.LabelFrame(scrollable_frame, text=" 🔒 Seguridad y Privacidad ", padx=15, pady=15)
            security_frame.pack(fill=tk.X, pady=(0, 15), padx=5)
            
            ttk.Label(security_frame, text="Encriptación de API Keys:").grid(row=0, column=0, sticky=tk.W, pady=5)
            encryption_active = hasattr(self, 'security') and self.security
            ttk.Label(security_frame, text="✅ Activa" if encryption_active else "⚠️ Inactiva", 
                           foreground="#10b981" if encryption_active else "#ef4444").grid(row=0, column=1, sticky=tk.W, padx=10)
            
            ttk.Label(security_frame, text="Backup de Pánico:").grid(row=1, column=0, sticky=tk.W, pady=(15, 5))
            ttk.Button(security_frame, text="🚨 Crear Backup", command=self.panic_backup).grid(row=1, column=1, sticky=tk.W, padx=10)
            
            self.audit_logging_var = tk.BooleanVar(value=getattr(self, 'audit_logging_enabled', True))
            audit_check = ttk.Checkbutton(security_frame, text="Auditoría de cambios", variable=self.audit_logging_var)
            audit_check.grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=(15, 5), padx=10)
            # === [MODULE: SECURITY_SETTINGS] END ===

            # === [MODULE: ADVANCED_NOTIFICATIONS] START ===
            notif_adv_frame = ttk.LabelFrame(scrollable_frame, text=" 🔔 Notificaciones Avanzadas (Elite) ", padx=15, pady=15)
            notif_adv_frame.pack(fill=tk.X, pady=(0, 15), padx=5)

            # === [INICIO FUNCIONALIDAD: PANEL DE CONFIGURACIÓN UI (US-CONFIG-PANEL)] ===
            if self.env_manager: # Feature 1: Panel de configuración UI
                self._setup_env_panel(scrollable_frame) 
            # === [FIN FUNCIONALIDAD: PANEL DE CONFIGURACIÓN UI] ===

            # Programación
            self.notif_sch_enabled = tk.BooleanVar(value=self.settings.get("notifications", {}).get("schedule_enabled", False))
            ttk.Checkbutton(notif_adv_frame, text="Habilitar Ventana Horaria", variable=self.notif_sch_enabled).grid(row=0, column=0, sticky=tk.W)
            
            time_frame = ttk.Frame(notif_adv_frame)
            ...
            ttk.Entry(time_frame, textvariable=self.notif_start_var, width=6).pack(side=tk.LEFT)
            ttk.Label(time_frame, text=" a ").pack(side=tk.LEFT)
            ttk.Entry(time_frame, textvariable=self.notif_end_var, width=6).pack(side=tk.LEFT)

            # Prioridad y Sonido
            ttk.Label(notif_adv_frame, text="Prioridad Mínima:").grid(row=1, column=0, sticky=tk.W, pady=10)
            self.notif_min_pri_var = tk.IntVar(value=self.settings.get("notifications", {}).get("min_priority", 3))
            ttk.Spinbox(notif_adv_frame, from_=1, to=5, textvariable=self.notif_min_pri_var, width=5).grid(row=1, column=1, sticky=tk.W)

            ttk.Label(notif_adv_frame, text="Sonido Personalizado:").grid(row=2, column=0, sticky=tk.W)
            self.notif_sound_var = tk.StringVar(value=self.settings.get("notifications", {}).get("custom_sound_path", ""))
            ttk.Entry(notif_adv_frame, textvariable=self.notif_sound_var, width=30).grid(row=2, column=1, sticky=tk.EW)
            ttk.Button(notif_adv_frame, text="📁", width=3, 
                      command=lambda: self.notif_sound_var.set(filedialog.askopenfilename(filetypes=[("WAV files", "*.wav")]))).grid(row=2, column=2, padx=5)

            # Botón Historial
            ttk.Button(notif_adv_frame, text="📜 Ver Historial (Últimas 100)", 
                      command=self._show_notification_history).grid(row=3, column=0, columnspan=2, pady=10, sticky=tk.EW)
            # === [MODULE: ADVANCED_NOTIFICATIONS] END ===
            
            # === MÓDULO 6: BOTONES DE ACCIÓN ===
            action_frame = ttk.Frame(scrollable_frame, padding=(0, 15))
            action_frame.pack(fill=tk.X)
            
            ttk.Button(action_frame, text="💾 Guardar Configuración", 
                      command=self._save_all_settings).pack(side=tk.RIGHT, padx=5)
            ttk.Button(action_frame, text="🔄 Restaurar Default", 
                      command=self._reset_settings).pack(side=tk.RIGHT, padx=5)

            # === MÓDULO 7: Bind mousewheel para scroll ===
            ...
            self.log("✅ Pestaña Configuración cargada (ttk + sv-ttk)")

            # === MÓDULO 8: Fallback visual (backup de funciones auxiliares) ===
            self._settings_filter_text = filter_text
            
        except Exception as e:
            # === FALLBACK VISUAL: Si algo falla, mostrar error amigable ===
            import traceback
            error_msg = f"Error cargando configuración:\n{str(e)}"
            self.log(f"❌ {error_msg}")
            
            # Frame de error
            error_container = ttk.Frame(self.tab_settings, padding=40)
            error_container.pack(fill=tk.BOTH, expand=True)
            
            ttk.Label(error_container, text="⚠️ Error al cargar Configuración", 
                     font=('Segoe UI', 14, 'bold'), foreground="#f59e0b").pack(pady=10)
            
            error_text = tk.Text(error_container, wrap=tk.WORD, height=8, font=('Consolas', 9))
    ...
    def _setup_env_panel(self, parent):
        ...
        env_frame = ttk.LabelFrame(parent, text=" 🌍 Configuración de Entorno (.env) ", padx=15, pady=15) # Feature 1: Panel UI
    ...
    def _show_notification_history(self):
        ...
        frame = ttk.Frame(win, padx=10, pady=10)
        frame.pack(fill=tk.BOTH, expand=True)
    ...
    def manage_channels(self):
        ...
        input_frame = ttk.LabelFrame(win, text=" Agregar / Editar Canal ", padx=10, pady=10)
        input_frame.pack(fill=tk.X)
        ...
        io_frame = ttk.Frame(win, padx=10, pady=10)
    ...
    def show_keybindings_editor(self):
        ...
        def edit_binding():
            ...
            lbl = ttk.Label(capture_win, text="Pulsa la nueva combinación de teclas...", padx=20, pady=20)
            lbl.pack()
        ...
        btn_frame = ttk.Frame(editor, padx=10, pady=10)
        btn_frame.pack(fill=tk.X)
    ...
    def merge_manual_content(self):
        ...
        stats_frame = ttk.LabelFrame(win, text=" Estado Actual de Manuales ", padx=10, pady=10)
        ...
        input_frame = ttk.LabelFrame(win, text=" Agregar Contenido Nuevo ", padx=10, pady=10)
        ...
        btn_frame = ttk.Frame(win, padx=10, pady=10)
        btn_frame.pack(fill=tk.X)
    ...
    def apply_specific_theme(self, theme_name):
        """Aplica un tema Bootstrap específico."""
        bootstrap_map = {"dark": "darkly", "light": "flatly"}
        theme = bootstrap_map.get(theme_name, theme_name)
    ...
    def setup_dashboard_tab(self):
        ...
        frame = ttk.Frame(self.tab_dashboard, padx=20, pady=20)
        frame.pack(fill=tk.BOTH, expand=True)
        ...
        # Info Server
        info_frame = ttk.LabelFrame(frame, text=" Estado del Servidor ", padx=20, pady=20)
