...
import shutil
import re
from convert_to_pdf import ExportConfig, convert_md_to_html, convert_md_to_pdf_with_styles, generate_toc, render_toc_html, rewrite_internal_links, resolve_images, ExportResult, get_weasyprint
import markdown2
...
SKIP_PDF = True  # WeasyPrint no disponible en Windows sin GTK3
...
class KBExporter:
    ...
    def export_single(self, md_path: Path) -> Dict:
        ...
        try:
            ...
            if not SKIP_PDF:
                from convert_to_pdf import get_weasyprint
                HTML = get_weasyprint()
                if HTML is not None:
...
<body>
    ...
    def export_all(self) -> Dict:
        ...
        for md_file in md_files:
            result = self.export_single(md_file)
