...
class KDPScoringService:
    ...
    def _detect_ai_generated_content(self, title: str, description: str = "") -> tuple[bool, float]:
        """
        Módulo B2: Detección de Contenido Generado por IA
        Analiza título y descripción para detectar patrones de contenido 100% IA.
        
        Returns:
            (is_ai_generated, confidence) - confidence 0.0-1.0
        """
        text = f"{title} {description}".lower()
        
        # Patrones que indican contenido generado por IA
        ai_patterns = [
            r'generated by ai',
        ...
        # Patrones que indican contenido auténtico (contrario)
        authentic_patterns = [
            r'experiencia personal',
            r'en mi caso',
            r'como yo',
            r'lo que hice',
            r'mi resultado',
        ...
        # Score de detección IA
        ai_score = 0
        for pattern in ai_patterns:
            if re.search(pattern, text):
                ai_score += 0.2
        
        # Reducir score si hay indicadores de autenticidad
        for pattern in authentic_patterns:
            if re.search(pattern, text):
                ai_score -= 0.15
        
        # Títulos muy cortos con patrones específicos
        if len(title) < 30 and any(w in text for w in ['tutorial', 'how to', 'cómo']):
            # Probablemente no es IA, es título simple
            ai_score -= 0.1
        
        # Normalizar a 0.0-1.0
        confidence = max(0.0, min(1.0, ai_score))
        
        # Umbral: más de 50% de confianza = detectado como IA
